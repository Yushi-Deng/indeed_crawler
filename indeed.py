...
def get_jobs_of_title(job_title):
    ...
    for i in range(num_pages): 
        ...
        for job_page_link in job_page_links:
            ...
            job_titles.append(data['job_title'])
            job_descrip.append(data['description'])
            # print(json.dumps(data))
    job_titles = pd.DataFrame(job_titles)
    job_descrip = pd.DataFrame(job_descrip)
    df = pd.concat([job_titles,job_descrip],axis = 1)
...
def get_info_from_job_page(url):
    ...
    data['description'] = soup.find('div', id = 'jobDescriptionText').getText()
    data["job_title"] = soup.find('div', class_='jobsearch-JobInfoHeader-title-container').find('h1').getText()
    # data["location"] = soup.find('div', class_="icl-u-lg-mr--sm icl-u-xs-mr--xs").find('a')
